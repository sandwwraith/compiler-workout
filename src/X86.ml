...
let rec compile env program = List.fold_left compile1 (env, []) program
  ...
  let ref_global e x = M ((e#global x)#loc x) in
...
let build stmt name =
  ...
  Sys.command (Printf.sprintf "gcc -m32 -Wl,-no_pie -o %s %s/runtime.o %s.s" name inc name)
