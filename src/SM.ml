...
let rec eval env ((controls, stack, ((state, input, output) as world)) as cfg) prog = match prog with
  ...
  | cmd :: rest -> 
    ...
    | CALL (f, _, _) -> eval env ((rest, state)::controls, stack, world) (env#labeled f)
...
let rec compile_expr expr = match expr with
    ...
    | Expr.Call (f, args) -> (List.flatten @@ List.map compile_expr (List.rev args)) @ [CALL (f, List.length args, true)]
in
